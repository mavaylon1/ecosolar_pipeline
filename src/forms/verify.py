...
JNB_SOURCE = {
    ...
    "property.job_city": "job: city (alone, for forms with a separate City box)",
    ...
    "owner.homeowner_city_zip": "contact: city, state_text, zip (combined, no street)",
    ...
    "applicant.applicant_street": "static (EcoSolar applicant contact) - street only, no city/state/zip",
    "applicant.applicant_city_zip": "static (EcoSolar applicant contact) - city/state/zip only, no street",
    ...
    "contractor.contractor_city_zip": "static (EcoSolar) - same as applicant.applicant_city_zip",
    "contractor.contractor_full_address": "static (EcoSolar) - contractor_address + contractor_city_zip combined",
    "contractor.contractor_email": "static (EcoSolar) - same as applicant.applicant_email",
    "contractor.contractor_phone_email": "static (EcoSolar) - contractor_phone + contractor_email combined",
    ...
    "solar.solar_kw_dc": "job: System size DC (pure, no AC-preferred fallback)",
    "solar.solar_kw_ac_only": "job: System_kw_ac (pure, no DC fallback)",
    ...
    "business.requires_building_permit": "business rule (always true on coarse Building-vs-Grading forms - solar/electrical work is never a grading permit)",
...
def build_rows(data: dict, mapping_path: Path, template_path: Path) -> list[list[str]]:
    ...
    for issue in issues:
        ...
        rows.append([pdf_field, issue.field, source, "", f"{issue.category}: {issue.message}"])
